...
function subscribeKPNS(regId,ostype)
{
    ...
    var sub = {"sid":regId,"appId":"100000002edca5207","ufid": "appukedarisetti@gmail.com","osType":ostype,"deviceId": deviceID };
...
function KPNSregCallback(status,result){
	 ...
	 ksid = tmp["ksid"];
		if(tmp["statusCode"] == 200){
			...
			alert("Failed to subscribe to KPNS Server!!" + tmp["message"]);
	}		
...
function fetchMessagesSucessCalllback(status, fetchMsgs) {
    if (status == 400) {
        ...
        kony.print("ERROR"+JSON.stringify(fetchMsgs));
        frmFetchMsgs.show();
        kony.application.dismissLoadingScreen();
    }
};
function fetchMessagesonClick(eventobject) {
	...
	var start= 1;
	var perpage = 10;
	var inp = {"ufid":mailId, "appId":kony.servicesapp.APP_ID, "startElement":start, "elementsPerPage":perpage};
	var myhttpheaders={"Content-Type":"application/json"};
	var paramTab = {postdata:JSON.stringify(inp),httpheaders:myhttpheaders};
...
function sendGeoLoction() {
   	...
   	var inp = {"ksid": ksid, "latitude": lati, "locname": "", "longitude": longi};
	var myhttpheaders={"Content-Type":"application/json"};
	var paramTab = {postdata:JSON.stringify(inp),httpheaders:myhttpheaders};
	...
	if(status==400){
	 ...
	 //alert("result:"+result);
		if((result["statusCode"] == 200))
			alert("Geolocation updated to KPNS sucessfully!!");
		else
			alert("Geolocation updated to KPNS Failed!!");
	}		
